...
4. **סיכום** — טקסט ארוך נשלח למודל עם הנחיה מפורשת לכתוב רק מה שמופיע בו. אם התוכן דל מדי, הוא מחזיר `NO_CONTENT` והפריט מושמט.
   ...
   **טקסט קצר מ-600 תווים מוצג כלשונו ולא מסוכם.** ציוץ הוא כבר תמציתי, וסיכום שלו רק מרחיק מהמקור. אם הוא כתוב בשפה אחרת מזו שביקשת, הוא מתורגם תרגום מלא ונאמן — בלי לקצר ובלי לשנות מספרים וסימולים.
5. **סידור** — כל הפריטים של הנושא מסודרים לפי זמן פרסום, מהחדש לישן, בלי קשר לסדר המקורות בקובץ. העדכון נקרא כציר זמן אחד.
6. **שליחה** — פריט עם תמונה נשלח כתמונה שהטקסט שלו הוא הכיתוב, כך שהגרף והניתוח שנכתב עליו מגיעים יחד ובמקום הנכון בציר הזמן. השאר נשלח כטקסט, מפוצל אוטומטית מתחת למגבלת 4096 התווים. טלגרם מוריד את התמונות בעצמו מהכתובת, ולכן אין בזה עלות.
